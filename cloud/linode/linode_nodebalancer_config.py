...
short_description: create / delete / update a config in a Linode NodeBalancer.
description:
  - Wrapper around the Linode NodeBalancer config API https://www.linode.com/api/nodebalancer or https://www.linode.com/docs/platform/nodebalancer/nodebalancer-reference-guide
...
options:
  ...
  protocol:
    ...
    choices: ['http', 'tcp']
    description:
      - The protocol of the config we are targeting. TODO: https is not currently supported (pull requests welcomed).
  algorithm:
    required: false
    type: string
    default: roundrobin
...
def linodeNodeBalancerConfigs(module, api, state, name, node_balancer_id,
    ...
    config = linode_find_nodebalancer_config(api, nodebalancer['NODEBALANCERID'], config_id,
                                      port, protocol)
        ...
        if state == "present":
            if config['PORT'] != port \
               ...
               or config['CHECK_BODY'] != str(check_body):
                ...
                changed = True
                config = linode_find_nodebalancer_config(api, nodebalancer['NODEBALANCERID'],
                                                  new['ConfigID'],
                                                  port, protocol)
        elif state == "absent":
    ...
    module.exit_json(changed=changed, instance=config, instances=[config])
