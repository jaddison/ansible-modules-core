...
short_description: create / delete / update nodes in a Linode NodeBalancer
description:
  - Wrapper around the Linode NodeBalancer node API https://www.linode.com/api/nodebalancer or https://www.linode.com/docs/platform/nodebalancer/nodebalancer-reference-guide
version_added: "1.9.4"
...
def linodeNodeBalancerNodes(module, api, state, name, node_balancer_id,
    ...
    node = linode_find_nodebalancer_node(api, config['CONFIGID'], node_id, node_name)
            ...
            if node['LABEL'] != node_name or node['ADDRESS'] != address or \
               node['WEIGHT'] != weight or node['MODE'] != mode:
                ...
                new = api.nodebalancer_node_update(NodeID=node['NODEID'],
                                                   ...
                                                   Mode=mode)
                changed = True
                node = linode_find_nodebalancer_node(api, config['CONFIGID'],
    ...
    module.exit_json(changed=changed, instance=node, instances=[node])
